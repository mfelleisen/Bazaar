...
(define-syntax (def/jsexpr-> stx)
  (syntax-parse stx
    ...
    [(_ to #:hash {{domain? (~optional (~seq #:parse parse) #:defaults ([parse #'id])) pat}} body ...)
     #:with name  (format-id stx "jsexpr->~a" #'to #:source #'to #:props stx)
     #:with jsexpr-> (let ([u #'parse]) (format-id u "jsexpr->~a" u #:source u #:props u))
     #:with keys (datum->syntax stx 'keys stx stx)
     #'(define name
         (λ (j)
           (match j
             [(hash-table [(? domain?) (app jsexpr-> pat)] (... ...))
              (define keys (map car (hash->list j)))
              body ...]
             [_ (eprintf "~a : object does not match ~a schema\n ~a\n" 'name 'to (jsexpr->string/ j))
                #false])))]))
...
(module+ test
  ...
  (define (color? x) (or (eq? x 'green) (eq? x 'red)))
  
  (def/jsexpr-> bar #:hash {[color? (and (? natural? n) (? positive?))]} (apply + n))
  (def/jsexpr-> tst #:object {['key aux (? natural? n)] ['key2 x]} (list (list 'key (list n x))))
  (def/jsexpr-> aux #:array [[list (? natural? n)] n])
  

  (check-equal? (jsexpr->bar (hash 'red 2 'green 1)) 3)
