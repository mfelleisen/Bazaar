...
(define (([make-remote-proxy-context make-dispatcher] receiver custodian) player)
  (define done? (box (gensym 'remote-is-done)))
...
(define-syntax (define-remote-proxy-context stx)
  (syntax-parse stx
    [[_ f f-s-class c:dispatcher-clause ...]
     #:with (m-names ...) (map extract-method-name (syntax->list #'[ c ...]))
     ...
     #'(begin
         (let ([class f-s-class])
           (unless (class? class)
             (error 'define-remote-proxy-context "class expected, given ~a" class))
           (define methods (interface->method-names (class->interface class)))
           (for ([m '(m-names ...)] #:unless (member m methods))
             (error 'define-remote-proxy-context "method name for ~a expected, given ~a" 'class m)))
         (define f
           (let ()
...
(define-for-syntax (extract-method-name stx)
  (syntax-parse stx
    [([method:id arg ...] return) #'method]))
